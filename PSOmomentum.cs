...
public class PSOmomentum : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < swarm.Length; ++i)
        {
            ...
            /*
            swarm[i].GetComponent<PSOagent>().minV = minV;
            swarm[i].GetComponent<PSOagent>().maxV = maxV;
            swarm[i].GetComponent<PSOagent>().minX = minX;
            swarm[i].GetComponent<PSOagent>().maxX = maxX
            */
            ;
    ...
    void update_swarm()
    {
        for (int i = 0; i < swarm.Length; ++i)
        {
            ...
            float mass = swarm[i].GetComponent<PSOagent>().mass;

            // without weighting, particles have issues stopping 
            //m.x = Time.deltaTime * (mWeight * (mass * currPR.velocity.x) + (1 - mWeight) * currPR.velocity.x);
            //m.z = Time.deltaTime * (mWeight * (mass * currPR.velocity.z) + (1 - mWeight) * currPR.velocity.z);
            ...
            //timeStep
            m.x = timeStep * (mWeight * (avg.x) + (1 - mWeight) * currPR.velocity.x);
            m.z = timeStep * (mWeight * (avg.z) + (1 - mWeight) * currPR.velocity.z);
            // store previous momentum to create the weighted average
            swarm[i].GetComponent<PSOagent>().mExpAvg.x = m.x;
            swarm[i].GetComponent<PSOagent>().mExpAvg.z = m.z;
            ...
            // timeStep
            swarm[i].transform.position = swarm[i].transform.position + timeStep * newV;
